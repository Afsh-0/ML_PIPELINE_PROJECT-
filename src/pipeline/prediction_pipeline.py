...
#create a function to load a object(pkl files)
#create a custome class based upon the dataset
...
class PredictionPipeline:
    ...
    def predict(self, features):
        ...
        model_path = os.path.join("artifacts/model_training", "model.pkl") #defined a path

        #now to load a pkl files we need a helper function so go to utils.py

        processor = load_object(preprocessor_path)
        load = load_object(model_path)

        scaled = processor.transform(features)
        pred = load.predict(scaled)
